 ...
 *  referenced in the document is collected. Each collected entry contains,
 *  along with the address itself, the in-memory reference to the DOM attribute
 *  or source fragment that will need modification when the new address for the
 *  resource is determined.
 *
 * The actual in-memory modification of the addresses generally happens after
 *  the resource has already been saved locally, since until then it is not
 *  known whether the save operation succeeded. Depending on the result of the
 *  save operation, the reference is replaced with a local relative URL or a
 *  full remote URI.
 *
...
ExactPersistParsedJob.prototype = {
   ...
   *  ExactPersistReference object, cross-referencing it with its source DOM
   *  node if required, and finally adding it to the references list for the
   *  document associated with this object.
   *
   * @param aProperties   Object containing the initial values of some of the
   *                       properties of the ExactPersistReference object to be
   *                       created. Other properties are set automatically
   *                       starting from the provided values. See the
   ...
   *  matches the given HTML element and attribute names. The element name can
   *  be "*" to indicate that any element is allowed, and the attribute name can
  ...
  _scanDomDocument: function() {
    ...
    for (let node in this._htmlNodesGenerator("script")) {
      ...
      //  scripting language specified in the "Content-Script-Type"
      //  header, and assumes JavaScript. For more information, see
  ...
  _scanCssStyleSheet: function(aStyleSheet) {
    ...
    //  objects allow the external web references in the stylesheet to be
    //  updated at a later time, before the output file is written to disk.
    var charsetRule = "";
   ...
   *
   * @param aFragment      SourceFragment object to be scanned.
   * @param aSaveLocally   If true, the resources will be saved locally.
   * @param aBaseUri       Optional nsIURI object used for reference resolution.
   */
