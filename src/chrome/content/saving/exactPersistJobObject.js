 ...
 *  one PersistResource object for each unique referenced resource, regardless
 *  of whether the resource should be saved locally or not. At the same time,
 *  each ExactPersistParsedJob object builds a list of ExactPersistReference
 *  objects, that associates every reference with its target PersistResource.
 *
 ...
 *  while the operation is in progress, the web references found in newly
 *  created DOM nodes or stylesheet rules will not be updated correctly.
 *
 * @param aDocument           The document to be saved, which will be inspected
 *                             to find additional related resources.
 * @param aTargetFile         The nsIFile where the main document will be saved.
 * @param aTargetDataFolder   The nsIFile of the support folder for data files.
 */
function ExactPersistJob(aEventListener, aDocument, aTargetFile,
 aTargetDataFolder, aSaveWithMedia, aSaveWithContentLocation) {
  ...
  //  _parsedJobs array. Create an unparsed job for all the resources that
  //  should be saved and don't have a parsed job already associated with them.
  ...
  for (var [curIndex, job] in Iterator(this._parsedJobs)) {
    ...
    //  since their content type is not known in advance, and the MIME media
    //  type may affect the file extension that is actually used.
    if (!curIndex) {
...
ExactPersistJob.prototype = {
   ...
   *
   * @param aReference      ExactPersistReference object to be registered with
   *                         this save job. The object will be updated with the
   *                         PersistResource object corresponding to the target.
   * @param aReferenceUri   nsIURI object pointing to the resource to be saved.
   *                         This URI usually corresponds to the target of the
   *                         reference, without the hash part.
   */
  ...
  setResourceLocation: function(aResource, aUriSpec) {
    ...
    if (this.saveWithContentLocation) {
      ...
      //  "%#;/?:@&=+$,[]" are URI-escaped using the UTF-8 character set. In
      //  this case, however, the character set is generally not relevant, as
      //  the URI specification is unlikely to contain international characters
      //  except when overriding the location of the main document. For more
      //  information on this escaping method, see the "EscapeURI" function in
   ...
   *  specified parameters, from the PersistBundle associated with this job. If
   *  a corresponding object is not already available, a new PersistResource
   *  object will be created and added to the PersistBundle.
   *
   * @param aUri        nsIURI object for the resource to be retrieved.
   * @param aModified   If true, indicates that a potentially modified version
   *                     of the original resource will be saved in place of the
   *                     original resource. This ensures that a unique reference
   *                     is returned even if a modified resource with the same
   *                     original URI already exists in the PersistBundle.
   */
  _getResourceForUri: function(aUri, aModified) {
    ...
    if (aModified) {
      ...
      //  coming from the specified URI. If a reference to an unmodified
      //  resource for the given URI already exists, it means that links to the
      //  resource were encountered previously. Since those links must be
      //  updated to point to the locally modified version of the resource,
      //  we reuse the corresponding resource object if possible. Conversely,
      //  we don't reuse resource objects corresponding to other modified
      //  resources, since we assume that every parsed version of the same
      //  original resource is unique.
   ...
   *  parsed content that may have been modified, and as such its unique URI
   *  does not correspond to the location the content was originally retrieved
   *  from. This allows multiple modified versions of the same original content
   *  to be present in the same web archive.
   */
